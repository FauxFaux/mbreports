...
$res = pg_query("select label.name, release.catno, release.barcode, album.name as albumname, artist.name as artistname, album.gid from release left join album on album.id=release.album left join artist on artist.id=album.artist left join label on label.id = release.label where length(barcode) =13 and barcode like '99%' order by label.name,catno desc");
